...
function initSchema(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS vcr1_partners (
      id INTEGER PRIMARY KEY,
      name TEXT NOT NULL,
      inn TEXT,
      legal_status TEXT,
    ...
    CREATE INDEX IF NOT EXISTS idx_vcr1_partners_phone ON vcr1_partners(phone);
    CREATE INDEX IF NOT EXISTS idx_vcr1_partners_name ON vcr1_partners(name);
    CREATE INDEX IF NOT EXISTS idx_vcr1_partners_inn ON vcr1_partners(inn);
    CREATE INDEX IF NOT EXISTS idx_vcr1_partners_company ON vcr1_partners(company);

    CREATE TABLE IF NOT EXISTS vcr1_licenses (
      id INTEGER PRIMARY KEY,
      partner TEXT,
      contract TEXT,
      created_at TEXT,
    ...
    CREATE INDEX IF NOT EXISTS idx_vcr1_licenses_partner ON vcr1_licenses(partner);
    CREATE INDEX IF NOT EXISTS idx_vcr1_licenses_serial ON vcr1_licenses(serial);
    CREATE INDEX IF NOT EXISTS idx_vcr1_licenses_license ON vcr1_licenses(license);
    CREATE INDEX IF NOT EXISTS idx_vcr1_licenses_created_at ON vcr1_licenses(created_at);
...
const upsertVcr1PartnerStmt = (db) =>
  db.prepare(`
    INSERT INTO vcr1_partners (
      id, name, inn, legal_status, phone, contacts, company, balance, registered_at, updated_at
    ) VALUES (
      @id, @name, @inn, @legal_status, @phone, @contacts, @company, @balance, @registered_at, datetime('now')
    )
    ON CONFLICT(id) DO UPDATE SET
      name = excluded.name,
      inn = excluded.inn,
...
function vcr1PartnerFromApiRow(row) {
  return {
    ...
    balance: row.balance ?? null,
    // vcr1 Partners/Index has no registration date column; Support still uses this field.
    registered_at: row.create_date ?? row.created_at ?? row.registered_at ?? null,
  };
}

function upsertVcr1Partners(db, partners) {
  const stmt = upsertVcr1PartnerStmt(db);
  db.exec('BEGIN');
  try {
    for (const row of partners) {
      stmt.run(row);
...
function countVcr1Partners(db) {
  return db.prepare('SELECT COUNT(*) AS count FROM vcr1_partners').get().count;
}

const upsertVcr1LicenseStmt = (db) =>
  db.prepare(`
    INSERT INTO vcr1_licenses (
      id, partner, contract, created_at, status, fm, serial, license,
      fda_version, app_build_time, db_version, last_receipt_date,
      last_check_attempt, last_sync, updated_at
    ) VALUES (
      @id, @partner, @contract, @created_at, @status, @fm, @serial, @license,
      @fda_version, @app_build_time, @db_version, @last_receipt_date,
      @last_check_attempt, @last_sync, datetime('now')
    )
    ON CONFLICT(id) DO UPDATE SET
      partner = excluded.partner,
      contract = excluded.contract,
      ...
      fda_version = excluded.fda_version,
      app_build_time = excluded.app_build_time,
      db_version = excluded.db_version,
      last_receipt_date = excluded.last_receipt_date,
      last_check_attempt = excluded.last_check_attempt,
...
function upsertVcr1Licenses(db, licenses) {
  const stmt = upsertVcr1LicenseStmt(db);
  db.exec('BEGIN');
  try {
    for (const row of licenses) {
      stmt.run(row);
...
function countVcr1Licenses(db) {
  return db.prepare('SELECT COUNT(*) AS count FROM vcr1_licenses').get().count;
}
