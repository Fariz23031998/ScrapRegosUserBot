...
function startOfLocalWeekMonday(date = new Date()) {
  const day = startOfLocalDay(date);
  const weekday = day.getDay(); // 0 Sun .. 6 Sat
  const daysFromMonday = weekday === 0 ? 6 : weekday - 1;
  return addLocalDays(day, -daysFromMonday);
}

function buildPresetReportPeriod(preset, now = new Date()) {
  const today = startOfLocalDay(now);

  if (preset === 'today') {
    const iso = toLocalIsoDate(today);
    return buildPeriodFromDates(iso, iso);
  }
  ...
  if (preset === 'this_week') {
    const from = startOfLocalWeekMonday(today);
    return buildPeriodFromDates(toLocalIsoDate(from), toLocalIsoDate(today));
  }

  if (preset === 'last_week') {
    const thisWeekStart = startOfLocalWeekMonday(today);
    const lastWeekStart = addLocalDays(thisWeekStart, -7);
    const lastWeekEnd = addLocalDays(thisWeekStart, -1);
    return buildPeriodFromDates(toLocalIsoDate(lastWeekStart), toLocalIsoDate(lastWeekEnd));
  }
  ...
  if (preset === 'last_month') {
    ...
    return buildPeriodFromDates(toLocalIsoDate(from), toLocalIsoDate(to));
  }
