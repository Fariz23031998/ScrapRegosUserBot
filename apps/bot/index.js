...
const { searchUser, looksLikePhone } = require('../../src/bot/search-user');
const {
  openDb,
  getBotUser,
  getEmployeeByPhone,
...
const { isLinkedEmployee } = require('../../src/db/user-rights');
const { syncUserCommands, getHelpCommandLines, buildDefaultBotCommands } = require('../../src/bot/bot-commands');
const { registerVipHandlers, handleVipMessage } = require('../../src/bot/vip-bot');
const { registerServiceHandlers, handleServiceMessage, makeServiceButtonForResult } = require('../../src/bot/service-bot');
const { registerReportHandlers, handleReportMessage } = require('../../src/bot/report-bot');
const {
  registerOrderActionHandlers,
  sendOrdersWithActions,
} = require('../../src/bot/order-actions-bot');
const { registerDashboardHandlers } = require('../../src/bot/dashboard-bot');
const {
...
async function sendCustomerPaymentLinks(chatId, phone) {
  ...
  const unpaidBlock = formatUnpaidOrdersBlock(orders.map((order) => enrichOrderParties(db, order)));
  if (!unpaidBlock) {
...
bot.on('message', async (msg) => {
  ...
  if (await handleTechnicalSupportMessage(bot, msg, botUser, db)) {
    return;
  }
...
bot
  .setMyCommands(buildDefaultBotCommands())
  .catch((error) => {
...
// A single failed Telegram/DB call must not take the long-running bot down.
process.on('unhandledRejection', (reason) => {
  const message = reason instanceof Error ? reason.message : String(reason);
  console.error('[bot] unhandled rejection:', message);
});
