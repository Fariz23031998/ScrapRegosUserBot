...
const { createOrder, getBotUsersByPhone } = require('./partners-db');
const { isLinkedEmployee } = require('./bot-users-db');
const { formatClickUrl } = require('./click');
const { formatPaymentPageUrl } = require('./payments-api');
const { enqueueOrderPaymentSms } = require('./sms-queue');
...
function formatOrderPaymentMessage(order, paymentPageUrl, paymentUrl, extraLine = '') {
  ...
  return [
    ...
    extraLine,
    '',
    paymentPageUrl ? `Страница оплаты: ${link}` : `Ссылка для оплаты: ${link}`,
  ]
    .filter(Boolean)
    .join('\n');
}
