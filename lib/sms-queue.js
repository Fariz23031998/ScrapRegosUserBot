...
async function enqueueOrderPaymentSms(db, order, paymentPageUrl) {
  ...
  try {
    const inviteMessage = formatSmsTelegramInviteMessage();
    let inviteJobId = null;
    if (inviteMessage) {
      const inviteJob = buildJob(inviteMessage);
      await enqueueSmsJob(inviteJob);
      inviteJobId = inviteJob.id;
    } else {
      console.warn('[SMS queue] TELEGRAM_BOT_USERNAME not set — skipping invite SMS');
    }

    const paymentJob = buildJob(formatSmsPaymentMessage(order, paymentPageUrl));
    await enqueueSmsJob(paymentJob);
    return {
      queued: true,
      jobId: paymentJob.id,
      inviteJobId,
