...
GETSMS is active only when `ENABLE_GETSMS=1` and both credentials are nonblank. It sends one message after order creation using `GETSMS_MESSAGE_TEMPLATE` (or the built-in default). Android, Eskiz, and MTProto can use their own templates (`SMS_GATEWAY_MESSAGE_TEMPLATE`, `ESKIZ_MESSAGE_TEMPLATE`, `TELEGRAM_MTPROTO_MESSAGE_TEMPLATE`); if those are unset they fall back to `GETSMS_MESSAGE_TEMPLATE`. `SMS_GATEWAY_ENABLED` controls only the Android path; enabling several transports intentionally sends through each. Failures are isolated and recorded as `sms_failed`.
